	...
	mRenderOptions(RenderOptions::TexturesAndFog)
{
	...
	XMStoreFloat4x4(&mWaterTexTransform, texScale);
...
bool BlendApp::Init(HINSTANCE hInstance)
{
...
void BlendApp::DrawScene()
{
	...
	case RenderOptions::Lighting:
		boxTech = mEffect->Light3Tech;
		landAndWavesTech = mEffect->Light3Tech;
		break;
	case RenderOptions::Textures:
		boxTech = mEffect->Light3TexAlphaClipTech;
		landAndWavesTech = mEffect->Light3TexTech;
		break;
	case RenderOptions::TexturesAndFog:
		boxTech = mEffect->Light3TexAlphaClipFogTech;
		landAndWavesTech = mEffect->Light3TexFogTech;
		break;
	}
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		mEffect->SetDiffuseMap(mBoxMapSRV);
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		mEffect->SetWorldInvTranspose(worldInvTranspose);
		mEffect->SetWorldViewProj(worldViewProj);
		mEffect->SetTexTransform(XMLoadFloat4x4(&mGrassTexTransform));
		mEffect->SetMaterial(mLandMat);
		mEffect->SetDiffuseMap(mGrassMapSRV);
		...
		mEffect->SetWorldInvTranspose(worldInvTranspose);
		mEffect->SetWorldViewProj(worldViewProj);
		mEffect->SetTexTransform(XMLoadFloat4x4(&mWaterTexTransform));
		mEffect->SetMaterial(mWavesMat);
		mEffect->SetDiffuseMap(mWavesMapSRV);
