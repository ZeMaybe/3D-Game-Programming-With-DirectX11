...
bool LightingApp::Init(HINSTANCE hinst)
{
	...
	mEffect = new DirPointSpotLightsEffect(md3dDevice, L"../FX/DirPointSpotLights.fxo");
	mInputLayout = mInputLayouts.InitLayout(md3dDevice, mEffect->LightTech, L"PosNormal");
...
void LightingApp::DrawScene()
{
	...
	mEffect->LightTech->GetDesc(&techDesc);
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		mEffect->LightTech->GetPassByIndex(p)->Apply(0, md3dImmediateContext);
...
void LightingApp::BuildLandGeometryBuffers()
{
	...
	vbd.ByteWidth = sizeof(Vertex::PosNormal) * grid.Vertices.size();
...
void LightingApp::BuildWaveGeometryBuffers()
{
	...
	vbd.ByteWidth = sizeof(Vertex::PosNormal) * mWaves.VertexCount();
	vbd.BindFlags = D3D11_BIND_VERTEX_BUFFER;
